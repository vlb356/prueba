...
const getCardClassName = (plan, isSelected) => {
  const base = 'rounded-2xl p-6 transition'
  const featured = plan.featured
    ? 'border border-accent-500/70 bg-gradient-to-b from-accent-500/20 to-accent-500/5 shadow-orange'
    : 'glass'
  const selected = isSelected ? 'ring-2 ring-accent-400/60' : ''

  return `${base} ${featured} ${selected}`.trim()
}

const getButtonClassName = (plan, isSelected) => {
  const base = 'mt-6 w-full rounded-xl px-4 py-2.5 text-sm font-semibold transition'
  ...
  if (plan.featured) {
    return `${base} bg-accent-500 text-primary-950 hover:bg-accent-400`
  }

  return `${base} border border-white/20 hover:border-accent-500/60`
}
    ...
    <section className="section-shell" id="pricing">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <article key={plan.name} className={getCardClassName(plan, isSelected)}>
              <h3 className="text-xl font-semibold">{plan.name}</h3>
              <p className="mt-3 text-4xl font-semibold">{plan.price}</p>
              <p className="mt-2 text-slate-300">{plan.description}</p>

              <button onClick={() => onSelectPlan(plan.name)} className={getButtonClassName(plan, isSelected)}>
