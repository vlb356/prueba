...
export function LeaguesSection({ onJoinLeague }) {
  ...
  const [newTournament, setNewTournament] = useState({ name: '', sport: 'Padel' })
  ...
  const globalStats = useMemo(
    () => ({
      ...
      liveLeagues: leagues.filter((league) => league.status === 'live').length,
      teams: leagues.reduce((acc, league) => acc + league.teams.length, 0),
    }),
    [leagues],
  )
  ...
  const registerTeam = () => {
    ...
    setLeagues((prev) =>
      prev.map((league) => (league.id !== selectedLeague.id ? league : { ...league, teams: [...league.teams, newTeam] })),
    )
  ...
  const createTournament = () => {
    ...
    const created = {
      ...
      prize: 'KR starter prize pack',
      teams: [],
      fixtures: [],
      bracket: { semifinals: [], final: { id: 'f', label: 'Final', a: 'TBD', b: 'TBD' } },
    }

    setLeagues((prev) => [created, ...prev])
    setStatusTab('upcoming')
    setSelectedLeagueId(created.id)
    setNewTournament({ name: '', sport: 'Padel' })
    onJoinLeague?.(`Tournament created: ${created.name}`)
  }
