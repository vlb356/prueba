...
const navItems = ['Hero', 'Features', 'AI visuals', 'Venues', 'Bookings', 'Community', 'Leagues', 'Pricing']
...
export default function App() {
  ...
  const submitLead = (event) => {
    ...
    notify(`Great! Pre-registration saved for ${leadForm.city} (${selectedPlan}).`)
  ...
  const handleBookVenue = (venueName) => {
    ...
    notify(`Booking request sent for ${venueName}.`)
  }
    ...
    <div className="overflow-hidden text-slate-100">
      ...
      <main className="space-y-24 pb-24 pt-8 sm:space-y-28">
        <HeroSection
          ...
          onDemo={() => document.getElementById('ai-visuals')?.scrollIntoView({ behavior: 'smooth' })}
        />
        <HowItWorks />
        <FeaturesGrid />
        <AIShowcase />
        <VenuesSection searchTerm={searchTerm} onBook={handleBookVenue} />
